...
`tsconfig.client.json` (DOM), `tsconfig.server.json` (`@cloudflare/workers-types`), and
`tsconfig.scripts.json` (Node) exist so DOM, Workers, and Node globals never leak into each
other. A file shared between projects must be added to each `include` list — this is why
`server/protocol.ts` and `shared/` appear in the client's config. `npm run typecheck` runs all
three; typechecking only one hides real breakage.
...
One Durable Object per Discord activity instance, keyed `idFromName(instanceId)`, so everyone in
a voice channel lands on the same room and a reconnect restores team and leader role. The DO uses
the WebSocket Hibernation API; identity is verified by the Worker (HMAC session token from
`server/session.ts`) and passed to the DO in `x-guessfi-user-*` headers, which are trustworthy
because a DO namespace is not publicly routable. Never trust identity from the client frame.
...
- Outside Discord (no `frame_id` param) the client falls back to `DiscordSDKMock`, and
  `/api/dev-session` mints a session for a fake user. That route only exists when
  `ALLOW_DEV_SESSIONS=true`, which lives in `.dev.vars` and is never deployed. Do not add it to
  `wrangler.jsonc`. `?user=alice&instance=room1` drives several tabs as different people.
