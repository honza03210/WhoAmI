...
describe('createSession / verifySession', () => {
  it('round-trips the user id', async () => {
    const token = await createSession(SECRET, USER_ID);
...
describe('verifySession rejects', () => {
  ...
  it('a tampered payload', async () => {
    ...
    const forgedPayload = Buffer.from(JSON.stringify({ uid: 'someone-else', exp: 2_000_000_000 }))
      .toString('base64url');
  ...
  it('an expired token', async () => {
    const token = await createSession(SECRET, USER_ID, -10);
